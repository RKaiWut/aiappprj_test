display_cols = {
    "age": "Age",
    "sex": "Sex",
    "trestbps": "Resting Blood Pressure",
    "chol": "Cholesterol",
    "fbs": "Fasting Blood Sugar",
    "thalach": "Maximum Heart Rate",
    "exang": "Exercise-Induced Angina",
    "oldpeak": "ST Depression",
    "slope": "ST Segment Slope",
    "ca": "Number of Major Vessels",
    "thal": "Thalassemia"
}

# Reverse the one-hot encoding
cp_labels = {
    1: "Typical Angina",
    2: "Atypical Angina",
    3: "Non-anginal Pain",
    4: "Asymptomatic"
}

restecg_labels = {
    0: "Normal",
    1: "ST-T Wave Abnormality",
    2: "Left Ventricular Hypertrophy"
}


# Convert raw prediction into output JSON
def format_prediction(raw_result, patient, top_n=5, min_impact=0.10):

    raw_shap = raw_result["shap_values"]

    combined = {}

    # Standard features
    for feature, display_name in display_cols.items():
        combined[display_name] = float(raw_shap.get(feature, 0))

    # Combine Chest Pain one-hot features
    cp_value = (raw_shap.get("cp_2.0", 0) + raw_shap.get("cp_3.0", 0) + raw_shap.get("cp_4.0", 0))

    combined[
        f"Chest Pain ({cp_labels[int(patient['cp'])]})"
        ] = float(cp_value)

    # Combine Rest ECG one-hot features
    ecg_value = (raw_shap.get("restecg_1.0", 0) + raw_shap.get("restecg_2.0", 0))

    combined[
        f"Resting ECG ({restecg_labels[int(patient['restecg'])]})"
    ] = float(ecg_value)

    # Sort by absolute impact
    factors = sorted(
        combined.items(),
        key=lambda x: abs(x[1]),
        reverse=True
    )

    # Remove tiny contributors
    factors = [
        {
            "feature": feature,
            "impact": round(value, 3),
            "direction": "increase" if value > 0 else "decrease"
        }
        for feature, value in factors
        if abs(value) >= min_impact
    ]

    # Limit displayed factors
    factors = factors[:top_n]

    return {
        "prediction": raw_result["prediction"],
        "risk_probability": raw_result["risk_probability"],
        "risk_percent": raw_result["risk_percent"],
        "top_factors": factors
    }


### PRINT FORMAT
# 0.1 Set risk threshold
def get_risk_level(probability):
    if probability < 0.3:
        return "Low"
    elif probability < 0.6:
        return "Medium"
    else:
        return "High"

# 0.2 List contributing Factors
def print_top_factors(factors):
    print("\nTop Contributing Factors")
    print("-" * 55)

    for factor in factors:
        arrow = "↑" if factor["direction"] == "increase" else "↓"

        print(
            f"{factor['feature']:<40}"
            f"{arrow} {factor['impact']:+.3f}"
        )

# Print
def pretty_print(result):
    probability = result["risk_percent"] / 100
    risk_level = get_risk_level(probability)

    print("\n========== CAD Risk Prediction ==========\n")

    print(f"CAD Risk Level  : {risk_level}")
    print(f"Risk Probability: {result['risk_percent']:.1f}%")

    print_top_factors(result["top_factors"])

    print("\n=========================================\n")